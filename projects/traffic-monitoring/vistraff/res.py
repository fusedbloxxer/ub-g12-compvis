...
class Assets(object):
    def __init__(self, assets_path: pb.Path, *args, **kwargs) -> None:
        super().__init__()
        ...
        self.weights_path = assets_path / 'weights'
    ...
    @staticmethod
    def read_static_bgs(dir: pb.Path) -> np.ndarray:
        bgs: t.List[np.ndarray] = []

        for index in range(1, 16):
            bg: np.ndarray = cv.imread(str(dir / f'context_{index}.png'))
